...
const handleSelectMachineType = async (machineType) => {
  setSelectingMachineType(machineType);
  setAssigningMachineType(machineType);
  setAvailableMachines([]);
  setSelectedMachine(null);

  const backendType = machineType === 'WASHING' ? 'WASHER' : 'DRYER';

  try {
    const machines = await machineService.getAvailableMachines(backendType);

    const normalized = Array.isArray(machines)
      ? machines.map((m) => ({
          id: m.id,
          type: m.type,
          capacity: m.capacity,
          ...
          usedByOrder: m.usedByOrder ?? null,
        }))
      : [];

    setAvailableMachines(normalized);
  } catch (error) {
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredOrders.map((order) => (
                      <tr key={order.id} className={`hover:bg-gray-50 ${order.status === 'READY_FOR_PICKUP' || order.status === 'COMPLETED' ? 'bg-yellow-50' : ''}`}>
                        <td className="px-6 py-4 whitespace-nowrap">#{order.id}</td>
                        <td className={`px-6 py-4 whitespace-nowrap ${order.status === 'READY_FOR_PICKUP' || order.status === 'COMPLETED' ? 'font-bold text-lg text-blue-700' : ''}`}>
                          {order.user?.name || 'N/A'}
                        </td>
